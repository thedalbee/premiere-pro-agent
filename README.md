...
- **Localhost-only bridge.** The daemon binds to `127.0.0.1:7201` (control) and `127.0.0.1:7300` (plugin WebSocket) — it never listens on a public interface. (Browser-origin rejection and a daemon↔plugin token handshake are planned hardening before the public release.)
